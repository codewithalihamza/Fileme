...
interface ReverseTaxCalculation {
  expectedMonthlyTax: number;
  requiredMonthlySalary: number;
  requiredYearlySalary: number;
  monthlyTax: number;
  yearlyTax: number;
  salaryAfterTax: number;
  yearlyIncomeAfterTax: number;
...
export const TaxCalculator = () => {
  ...
  const [reverseCalculation, setReverseCalculation] =
    useState<ReverseTaxCalculation | null>(null);
  ...
  const [displayTargetValue, setDisplayTargetValue] = useState<string>("");
  ...
  const handleTargetInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setDisplayTargetValue(formatNumberWithCommas(value));
    // Close results when input changes
    setCalculation(null);
    setReverseCalculation(null);
  };
  ...
  const handleCalculate = async () => {
    if (isReverseMode) {
      await handleReverseCalculate();
    } else {
      await handleForwardCalculate();
    }
  };
  ...
  const handleReverseCalculate = async () => {
    const expectedTax = parseFloat(targetAmount);
    if (isNaN(expectedTax) || expectedTax < 0) {
      setReverseCalculation(null);
      return;
    }
    ...
    const requiredYearlySalary = calculateRequiredIncome(
      expectedTax,
      selectedTaxYear.year
    );
    const requiredMonthlySalary = requiredYearlySalary / 12;

    // Verify the calculation by using the forward calculation
    const calculatedYearlyTax = calculateTax(
      requiredYearlySalary,
      selectedTaxYear.year
    );
    const calculatedMonthlyTax = calculatedYearlyTax / 12;
    const salaryAfterTax = requiredMonthlySalary - calculatedMonthlyTax;
    const yearlyIncomeAfterTax = requiredYearlySalary - calculatedYearlyTax;

    // Ensure we have valid results
    if (requiredYearlySalary <= 0 || requiredMonthlySalary <= 0) {
      setReverseCalculation(null);
      setIsCalculating(false);
      return;
    }

    // Ensure we're using the exact expected tax amount for display
    setReverseCalculation({
      expectedMonthlyTax: expectedTax,
      requiredMonthlySalary: Math.round(requiredMonthlySalary),
      requiredYearlySalary: Math.round(requiredYearlySalary),
      monthlyTax: expectedTax, // Use the exact expected tax
      yearlyTax: expectedTax * 12, // Use the exact expected yearly tax
      salaryAfterTax: Math.round(salaryAfterTax),
        ...
        <div className="mx-auto mt-10 max-w-5xl">
          ...
          >
            ...
            <motion.div
              ...
              viewport={{ once: true }}
            >
              <div className="flex items-center gap-4 rounded-full border border-gray-200 bg-gray-50 p-1">
                <button
                  onClick={() => isReverseMode && toggleMode()}
            ...
            >
              ...
              >
                {isReverseMode
                  ? "Calculate Salary for Expected Monthly Tax"
              ...
              >
                <div className="flex-1">
                  ...
                  >
                    <Input
                      id={isReverseMode ? "target-amount" : "monthly-salary"}
                      type="text"
                      placeholder={
                        isReverseMode
                      ...
                      value={isReverseMode ? displayTargetValue : displayValue}
                      onChange={
                        isReverseMode
                  ...
                  <AnimatePresence mode="wait">
                    {isCalculating ? (
                      ...
                      <motion.span
                        ...
                        exit={{ opacity: 0 }}
                      >
                        {isReverseMode ? "Calculate Salary" : "Calculate Tax"}
                      </motion.span>
                    )}
                  </AnimatePresence>
                </Button>
                  ...
                  >
                    <Card className="border-green-200 bg-gradient-to-br from-green-50 to-emerald-50 p-8 shadow-xl">
                      ...
                      >
                        {[
                          {
                            label: "Required Monthly Salary",
                            value: reverseCalculation.requiredMonthlySalary,
                            color: "green",
                          },
                          {
                            label: "Monthly Tax",
                            value: reverseCalculation.monthlyTax,
                            color: "red",
                          },
                          {
                            label: "Salary After Tax",
                            value: reverseCalculation.salaryAfterTax,
                            color: "green",
                            highlight: true,
                          },
                        ].map((item, index) => (
                  ...
                  >
                    <Card className="border-blue-200 bg-gradient-to-br from-blue-50 to-indigo-50 p-8 shadow-xl">
                      ...
                      >
                        {[
                          {
                            label: "Required Yearly Salary",
                            value: reverseCalculation.requiredYearlySalary,
                            color: "blue",
                          },
                          {
                            label: "Yearly Tax",
                            value: reverseCalculation.yearlyTax,
                            color: "red",
                          },
                          {
                            label: "Income After Tax",
                            value: reverseCalculation.yearlyIncomeAfterTax,
                            color: "blue",
                            highlight: true,
                          },
                        ].map((item, index) => (
